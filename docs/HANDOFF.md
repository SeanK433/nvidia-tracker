...
The project follows two implementation plans (already executed):
...
Design spec: `docs/superpowers/specs/2026-04-28-nvidia-tracker-design.md`.

---

## Current state

Both plans have been executed and deployed. The dashboard is live and renders 28 partners across 6 cluster columns arranged around a central NVIDIA bubble. The most recent visual polish work cleaned up logo cropping, label backgrounds, layout positioning, and the NVIDIA center bubble.
...
- **Vitest** for tests (35 tests passing)
...
│   │   ├── filters.ts         # filterActive, filterByCategory, sortByPartner, etc.
│   │   └── schema.ts          # Zod schemas for Relationship, PendingProposal, Article, RawFile
│   └── styles/
│       └── theme.css          # design tokens, flat #f3ede0 background
├── .claude/
│   └── commands/
│       └── extract.md         # /extract slash command for weekly extraction
├── .github/workflows/
...
### 6. Multi-column overflow when a cluster has >4 partners

`computePositions()` packs partners into vertical sub-columns of max 4 rows. Vertical cluster (8 partners) → 2 sub-columns of 4. Silicon/cloud (5 partners) → 4 + 1. `SUB_COL_SPACING` was tuned to 120px to avoid overlap with current logo sizes; if you change logo widths, you may need to retune.
...
GitHub Action `.github/workflows/reminder.yml` at 9am ET. Calls `scripts/send-reminder.ts`. Counts articles since the last extraction commit. Sends email via Resend (uses `RESEND_API_KEY` and `EMAIL_TO` GitHub secrets).

### Weekly (manual, ~10 min)
User opens project in Claude Code, types `/extract`. The slash command (`.claude/commands/extract.md`) instructs Claude to:
1. Read `CATEGORIES.md`, `extract_prompt.md`, current `data/relationships.json`
2. Find all `raw/*.json` files newer than the last extraction commit
3. Classify each article: REJECT / UPDATE / PROPOSE NEW
4. Auto-apply UPDATEs to `relationships.json` (append URL to evidence_history, bump last_confirmed)
5. Stage PROPOSE NEWs in `pending.json`
6. Walk user through proposals one at a time (`keep` / `skip` / `edit`)
7. Commit with format: `Week of YYYY-MM-DD: N updates, M new partners` — the "Week of YYYY-MM-DD" prefix is required because send-reminder.ts greps for it to compute days-since.
8. User runs `npm run deploy` to push the rebuilt site
...
- Initial seed pass already populated 28 partners
...
- "Run the weekly extraction" — type `/extract`. Walk through the proposals.
...
3. **Tests:** `npm test` — should still be 35 passing.
